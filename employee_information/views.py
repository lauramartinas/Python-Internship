...
def total_salary(request):
    ...
    try:    
        bonus = Employees.objects.get(person_of=request.user).bonus
    except Employees.DoesNotExist:
        bonus = 0

    salary = Employees.objects.filter(person_of=request.user).salary
    salary = 0
    for f in salary:
        salary += f.bonus
...
def export_employee_csv(request):
    ...
    employees = Employees.objects.all().values_list('code', 'firstname', 'lastname', 'email', 'department_id', 'position_id',)
...
def search(request):
    if request.method =="POST":
        ...
        return render(request, 'employee_information/search.html', {'searched':searched, 'employees':employees})
    else:
       return render(request, 'employee_information/search.html') 
